...
const LINKEDIN_SELECTORS = {
    POST: '.update-components-text, .feed-shared-update-v2, .update-components-article, .feed-shared-article',
    ACTIONS: '.update-components-action-bar, .feed-shared-social-action-bar, .social-actions, .feed-shared-social-actions, .feed-shared-update-v2__social-actions, .feed-shared-update-v2__social-bar',
    POST_CONTENT: '.update-components-text__text-view, .feed-shared-text, .update-components-text, .feed-shared-update-v2__description'
};
...
function handleError(message, error = null) {
    ...
    // Only show toast for user-initiated actions that fail
    if (message.includes('No post content found')) {
        showToast('EMPTY_POST');
    }
    // Don't show unknown error toasts during initialization or background processes
}
...
function insertRoastButton(post) {
    try {
        ...
        if (!actionsContainer) {
            // Silently return false - no need to warn since this is expected for some post types
            return false;
        }
        ...
        if (!(error instanceof DOMException)) {
            console.warn('Failed to insert roast button:', error);
        }
        return false;
    }
}
...
chrome.storage.local.get(['roasterEnabled'], function(result) {
    ...
    if (result.roasterEnabled === true) {
        // Wait for DOM to be ready
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initializeRoaster);
        } else {
            initializeRoaster();
        }
    }
...
chrome.storage.onChanged.addListener((changes, namespace) => {
    if (changes.roasterEnabled?.newValue !== undefined) {
        if (changes.roasterEnabled.newValue === true) {
            // Wait for DOM to be ready
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', initializeRoaster);
            } else {
